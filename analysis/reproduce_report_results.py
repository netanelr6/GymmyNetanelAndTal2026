...
5. failure_detection_coding_final.csv
...
computed from the four raw workbooks, except the binary failure-detection classification,
which is a documented manual coding decision stored in failure_detection_coding_final.csv.
"""
...
DETECTION_CODING = "failure_detection_coding_final.csv"
...
def load_detection_coding(data_dir: Path) -> list[dict[str, str]]:
    path = data_dir / DETECTION_CODING
    with path.open(encoding="utf-8-sig", newline="") as file:
        rows = list(csv.DictReader(file))
    if sorted(int(row["ID"]) for row in rows) != VALID_IDS:
        raise ValueError("Failure-detection coding file does not contain exactly the 40 valid participants.")
    return rows
...
def compute_anova(clean_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    outcome = values(clean_rows, "Overall acceptance change")
    ...
    cell_values = [
        values(clean_rows, "Overall acceptance change", **{"Failure type": failure_type, "Timing": timing})
...
def create_figures(
    ...
    # Figure 10: Overall acceptance control/failure
    control = values(clean_rows, "Overall acceptance control")
    failure = values(clean_rows, "Overall acceptance failure")
    ...
    save(fig, "figure_08_overall_acceptance_control_vs_failure.png")
    ...
    # Figure 13: type
    hardware = values(clean_rows, "Overall acceptance change", **{"Failure type": "Hardware"})
    interaction = values(clean_rows, "Overall acceptance change", **{"Failure type": "Interaction"})
    fig, ax = plt.subplots(figsize=(8.2, 5.8))
    ax.boxplot(
        [hardware, interaction],
    ...
    # Figure 14: timing
    early = values(clean_rows, "Overall acceptance change", Timing="Early")
    late = values(clean_rows, "Overall acceptance change", Timing="Late")
    fig, ax = plt.subplots(figsize=(8.2, 5.8))
    ax.boxplot(
        [early, late],
...
def create_appendix_docx(
    ...
    paragraph = document.add_paragraph(
        ...
        "variable was failure detection, for which the participant-tracking log and the "
    ...
    add_table(
        ...
        [
            ...
            ["Overall Acceptance", "(Usefulness + Ease of Use + Attitude + Intention to Use) / 4"],
    ...
    add_heading(document, "E.4 Failure Detection and Recovery", 2)
    add_table(
        document,
        "Table E.4. Pearson chi-square tests for failure detection",
        ["Comparison", "Detected counts", "Not detected counts", "chi-square(df)", "p", "Phi"],
        [
            [
                row["Comparison"],
                row["Detected counts"],
                row["Not detected counts"],
    ...
    add_table(
        document,
        "Table E.5. Failure detection by experimental group and successful recovery",
        ["Group", "Failure type", "Timing", "Detected", "Not detected", "Detection rate"],
        [
            [
                row["Group"],
                row["Failure type"],
    ...
    document.add_paragraph(
        ...
        "participants classified as detecting the failure (5.3%). Participant-level coding "
    ...
    add_table(
        document,
        "Table E.6. Descriptive statistics for Overall Acceptance change scores",
    ...
    add_table(
        document,
        "Table E.7. Two-way ANOVA on Overall Acceptance change scores",
    ...
    document.add_page_break()
    add_heading(document, "Appendix F - Participant-Level Failure-Detection Coding", 1)
    document.add_paragraph(
        "Detection was coded from the participant-tracking log and the post-session questionnaire. "
    ...
    add_table(
        document,
        "Table F.1. Final participant-level failure-detection coding",
        ["ID", "Group", "Type", "Timing", "Detected", "Evidence source", "Original-file location", "Coding rationale"],
        f_rows,
        font_size=6.6,
    )
    ...
    document.add_paragraph(
        ...
        "raw workbooks and the final detection-coding CSV regenerates all statistics, appendix tables, "
    ...
    add_table(
        ...
        [
            ...
            ["5.2 Sample and prior attitudes", "load_pre_experience; descriptive mean and sample SD", "E.2"],
            ["5.3 Overall effect of failure", "paired_test_row", "E.3"],
            ["5.4 Detection and recovery", "Pearson chi-square contingency tables; manual coding input", "E.4-E.5 and F.1"],
...
def create_cross_reference_docx(output_dir: Path) -> Path:
    ...
    entries = [
        ...
        (
            "Section 5.4 - after the detection statistics and recovery paragraph",
            "The complete contingency tables and group-level detection results are provided in Appendix E, Tables E.4-E.5; participant-level coding and evidence locations are provided in Appendix F, Table F.1.",
        ),
        (
...
def create_missing_values_docx(output_dir: Path, paired_rows: list[dict[str, Any]], descriptive_rows: list[dict[str, Any]], anova_rows: list[dict[str, Any]], detection_tests: list[dict[str, Any]]) -> Path:
    ...
    entries = [
        ...
        (
            ...
            f"Control M = {paired['Overall acceptance']['Control M']:.2f}, SD = {paired['Overall acceptance']['Control SD']:.2f}; failure M = {paired['Overall acceptance']['Failure M']:.2f}, SD = {paired['Overall acceptance']['Failure SD']:.2f}; mean change = {paired['Overall acceptance']['Mean change']:.2f}; t(39) = {paired['Overall acceptance']['t']:.2f}, p = {paired['Overall acceptance']['p']:.3f}.",
        ),
        (
            "5.4 Detection",
            f"Failure type: chi-square(1) = {detection['Failure type']['Chi-square']:.2f}, p = {detection['Failure type']['p']:.3f}, phi = {detection['Failure type']['Phi']:.2f}. Failure timing: chi-square(1) = {detection['Failure timing']['Chi-square']:.2f}, p = {detection['Failure timing']['p']:.3f}. Recovery: 1/40 = 2.5%; 1/19 detectors = 5.3%.",
        ),
        (
            "5.5 Failure Type",
...
def main() -> None:
    ...
    shutil.copy2(data_dir / DETECTION_CODING, output_dir / DETECTION_CODING)
    ...
    # Detection tests, without continuity correction, matching the report.
    detection_tests: list[dict[str, Any]] = []
        ...
        detection_tests.append(
            {
                ...
                "Detected counts": "; ".join(detected_counts),
                "Not detected counts": "; ".join(not_detected_counts),
        ...
        group_detection_rows.append(
            {
                ...
                "Detected": detected,
                "Not detected": len(selected) - detected,
                "Detection rate": 100 * detected / len(selected),
            }
        )
    total_detected = sum(row["Detected"] == "Yes" for row in detection_rows)
    group_detection_rows.append(
        {
            "Group": "Total",
            "Failure type": "",
            ...
            "Not detected": len(detection_rows) - total_detected,
            "Detection rate": 100 * total_detected / len(detection_rows),
        }
    )
    ...
    for comparison_level, field, conditions in [
        ...
        for condition in conditions:
            group_values = values(clean_rows, "Overall acceptance change", **{field: condition})
    ...
    write_csv(output_dir / "table_E2_sample_and_prior_attitudes.csv", pre_statistics)
    write_csv(output_dir / "table_E3_paired_tests.csv", paired_rows)
    write_csv(output_dir / "table_E4_detection_chi_square.csv", detection_tests)
    write_csv(output_dir / "table_E5_detection_by_group.csv", group_detection_rows)
    ...
    report_values = {
        ...
        "detection_by_group": group_detection_rows,
        "successful_recovery": {
            "count": 1,
            "full_sample_percentage": 100 / 40,
            "detectors_percentage": 100 / total_detected,
        },
