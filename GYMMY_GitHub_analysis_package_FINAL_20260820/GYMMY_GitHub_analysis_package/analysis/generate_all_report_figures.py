...
def make_all_figures(data_dir: Path, output_dir: Path) -> None:
    ...
    recognition_rows = read_rows(data_dir / "failure_recognition_coding_public.csv")
    ...
    # Figure 8: Acceptance in control and failure sessions.
    control = numeric(clean_rows, "Acceptance control")
    failure = numeric(clean_rows, "Acceptance failure")
    ...
    save(fig, output_dir, "figure_08_acceptance_control_vs_failure.png")

    # Figures 9-10: failure recognition by type and timing.
    recognition_specs = [
        (
            "Failure type",
    ...
    for field, categories, labels, xlabel, filename in recognition_specs:
        percentages = []
        counts = []
        for category in categories:
            selected = [row for row in recognition_rows if row[field] == category]
            recognized = sum(row["Recognized"] == "Yes" for row in selected)
            percentages.append(100.0 * recognized / len(selected))
            counts.append(f"{recognized}/{len(selected)}")
    ...
    for failure_type in failure_types:
        ...
        for timing in timings:
            ...
            values = numeric(selected, "Acceptance change")
